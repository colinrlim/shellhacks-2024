...
// Function types
type OnAnswerReceiveProcessedDataType = (question_response: QuestionResponse_T) => void;
type OnFavoriteReceiveProcessedDataType = (question_response: QuestionResponse_T) => void;
type OnQuestionCreateReceiveDataType = (question: Question_T) => void;
type OnRegisterTopicReceiveDataType = (topic_name: string, topic_description: string | null) => void;
type OnRegisterRelationshipReceiveDataType = (prereq_topic_name: string, child_topic_name: string, strength: number) => void;
type OnExplanationReceiveDataType = (explanation: string) => void;
...
async function INPUT_start_session(query: string) {
    const messages = [
...
async function INPUT_answer(question: Question_T, answer: Response_T) {
    ...
    functions.onAnswerReceiveProcessedData(current_response);
    _send(messages);
}
async function INPUT_favorite(question_response: QuestionResponse_T) { // TODO: Add answers to favorites metadata
    ...
    functions.onFavoriteReceiveProcessedData(question_response);
    _send(messages);
}
...
async function _send(messages: any) { // DONE
...
function _do_calls(function_calls: _FunctionCallUnparsed_Bot[]): void { // DONE
    ...
    for (let i = 0; i < function_calls.length; i++) {
        ...
        if (function_call.name == "create_multiple_choice_question") {  // Create question DONE
            ...
            functions.onQuestionCreateReceiveData(_deep_copy(new_question));
            ...
            if (function_call.arguments.hasOwnProperty("prerequisite_topics")) { // add established topic as child topic of all of these
                for (let i = 0; i < function_call.arguments.prerequisite_topics.length; i++) {
                    ...
                    functions.onRegisterTopicReceiveData(curr_topic.name, curr_topic.description);
                    functions.onRegisterRelationshipReceiveData(curr_topic.name, function_call.arguments.name, curr_topic.strength);
                }
            }
            if (function_call.arguments.hasOwnProperty("child_topics")) { // add all of these as child topics of established topic
                for (let i = 0; i < function_call.arguments.child_topics.length; i++) {
                    ...
                    functions.onRegisterTopicReceiveData(function_call.arguments.name, function_call.arguments.description);
                    functions.onRegisterRelationshipReceiveData(function_call.arguments.name, curr_topic.name, curr_topic.strength);
                }
            }
            ...
            functions.onRegisterTopicReceiveData(function_call.arguments.prerequisite_topic, null);
            functions.onRegisterRelationshipReceiveData(function_call.arguments.prerequisite_topic, function_call.arguments.child_topic, function_call.arguments.strength);
        } else if (function_call.name == "explain_question") {
            functions.onExplanationReceiveData(function_call.arguments.explanation);
        }
    }
}
