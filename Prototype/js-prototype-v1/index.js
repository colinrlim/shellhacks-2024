  ...
  registered_topics: {
    ...
    value: [
      ...
      },
      {
        name: "Javascript",
        description:
          "A versatile programming language primarily used for web development, enabling interactive web pages.",
        relationships: [],
      },
      {
        name: "Programming",
...
function do_calls(function_calls) {
  // DONE
  ...
  for (let i = 0; i < function_calls.length; i++) {
    ...
    if (function_call.name == "create_multiple_choice_question") {
      ...
      assert.strictEqual(
        metadata.registered_topics.value.hasOwnProperty(
...
async function answer(questionID, answer) {
  ...
  current_question.user_response.selected_choice_content =
    current_question.question_data["choice_" + answer];
  current_question.user_response.is_correct =
    current_question.question_data.correct_choice == answer;
  ...
  if (current_question.user_response.is_correct) {
    ...
    console.log(
      `You were wrong. The correct answer was Choice ${current_question.question_data.correct_choice}.`
    );
  }
  ...
  const messages = [
    {
      role: "system",
      content: JSON.stringify({ current_question: current_question }),
    },
    {
      role: "system",
      content:
...
function favorite(questionID) {
  ...
  const messages = [
    {
      role: "system",
      content: JSON.stringify({ recent_favorite: questions[questionID] }),
    },
    {
      role: "system",
      content:
