...
type OnFavoriteReceiveProcessedDataType = (
  uid: string,
  session_id: string,
  question_response: QuestionResponse_T
) => void;
type OnQuestionCreateReceiveDataType = (
  uid: string,
  session_id: string,
  question: Question_T
) => void;
type OnRegisterTopicReceiveDataType = (
  uid: string,
  session_id: string,
  opic_name: string,
  topic_description: string | null
) => void;
type OnRegisterRelationshipReceiveDataType = (
  uid: string,
  session_id: string,
  prereq_topic_name: string,
  child_topic_name: string,
  strength: number
) => void;
type OnExplanationReceiveDataType = (
  uid: string,
  session_id: string,
  explanation: string
) => void;
...
const functions = {
  ...
  onFavoriteReceiveProcessedData:
    (() => {}) as OnFavoriteReceiveProcessedDataType,
...
export const setOnAnswerReceiveProcessedData = (
  fn: OnAnswerReceiveProcessedDataType
) => {
  functions.onAnswerReceiveProcessedData = fn;
};
export const setOnFavoriteReceiveProcessedData = (
  fn: OnFavoriteReceiveProcessedDataType
) => {
  functions.onFavoriteReceiveProcessedData = fn;
};
export const setOnQuestionCreateReceiveData = (
  fn: OnQuestionCreateReceiveDataType
) => {
  functions.onQuestionCreateReceiveData = fn;
};
export const setOnRegisterTopicReceiveData = (
  fn: OnRegisterTopicReceiveDataType
) => {
  functions.onRegisterTopicReceiveData = fn;
};
export const setOnRegisterRelationshipReceiveData = (
  fn: OnRegisterRelationshipReceiveDataType
) => {
  functions.onRegisterRelationshipReceiveData = fn;
};
export const setOnExplanationReceiveData = (
...
export const setSendMetadataFromDatabases = (
  ...
  functions.sendMetadataFromDatabases = fn;
};

export { INPUT_start_session, INPUT_answer, INPUT_favorite };
...
async function INPUT_start_session(
  uid: string,
  session_id: string,
  query: string
) {
  ...
  const messages = [
    {
      ...
      content: JSON.stringify({
        current_question: _transform_question_response(current_response),
      }),
    },
    {
      role: "system",
...
async function _send(uid: string, session_id: string, messages: any) {
  ...
  while (!sufficient && (result == undefined || result == null)) {
    ...
    if (result) {
      ...
      if (
        result.findIndex(
          (obj) => obj.function.name == "create_multiple_choice_question"
        ) != -1
      ) {
        sufficient = true;
      }
  ...
  for (let i = 0; i < function_calls.length; i++) {
    ...
    if (function_call.name == "create_multiple_choice_question") {
      ...
      if (function_call.arguments.hasOwnProperty("prerequisite_topics")) {
        ...
        for (
          let i = 0;
          i < function_call.arguments.prerequisite_topics.length;
          i++
        ) {
      ...
      if (function_call.arguments.hasOwnProperty("child_topics")) {
      ...
      functions.onRegisterTopicReceiveData(
        uid,
        session_id,
        function_call.arguments.prerequisite_topic,
        null
      );
      functions.onRegisterRelationshipReceiveData(
        uid,
        session_id,
        function_call.arguments.prerequisite_topic,
        function_call.arguments.child_topic,
      ...
      functions.onExplanationReceiveData(
        uid,
        session_id,
        function_call.arguments.explanation
      );
    }
  }
}
...
function _transform_question_response(
  question_response: QuestionResponse_T
): _QuestionResponse_Bot {
  return {
    description:
    ...
    user_response: {
      description:
        "This includes the number of the choice that the user chose, including what that choice said. For good measure, this also includes a boolean value that represents whether the user got the question correct or not.",
      selected_choice: question_response.user_response.selected_choice,
      selected_choice_content:
        question_response.user_response.selected_choice_content,
      is_correct: question_response.user_response.is_correct,
    },
  };
}
function _transform_question_response_array(
  question_response_array: QuestionResponse_T[]
): _QuestionResponse_Bot[] {
